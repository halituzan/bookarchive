...
import PublisherTypes from "../../models/Enums/publisherTypes.model";

export const getPublisherTypes = async (req: Request, res: Response) => {
  ...
  try {
    const data = await PublisherTypes.aggregate([
      { $match: matchStage },
    ...
    // Toplam kitap sayısı
    const total = await PublisherTypes.countDocuments(matchStage);
